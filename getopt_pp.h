...
struct Token {
  ...
  void link_to(Token *new_next) { next = new_next; }
...
struct OptionData {
  enum _Flags { CmdLine_NotExtracted, CmdLine_Extracted, Envir };
...
struct _Option {
  ...
  virtual Result operator()(ShortOptions &short_ops, LongOptions &long_ops,
                            Token *first, std::ios::fmtflags flags) const = 0;
  virtual ~_Option() {}

  static const char NO_SHORT_OPT = 0;

 protected:
  static void setTokenAsUsed(Token *token, ShortOptions &short_ops,
                             Token::Type usedAs) {
...
template <class T>
inline _Option::Result convert(const std::string &s, T &result,
...
template <>
inline _Option::Result convert<std::string>(const std::string &s,
                                            std::string &result,
                                            std::ios::fmtflags /*flags*/) {
  result = s;
  return _Option::OK;
...
class _OptionTBase : public _Option {
 ...
 protected:
  T &target;
  virtual Result _assign(Token *token, std::ios::fmtflags flags,
                         ShortOptions &short_ops) const = 0;
 ...
 public:
  ...
  virtual Result operator()(ShortOptions &short_ops, LongOptions &long_ops,
                            Token * /*first*/, std::ios::fmtflags flags) const {
...
class _OptionT : public _OptionTBase<T> {
 protected:
  virtual _Option::Result _assign(Token *token, std::ios::fmtflags flags,
                                  ShortOptions &short_ops) const {
 ...
 public:
  _OptionT(const _OptionT<T> &other) : _OptionTBase<T>(other) {}
  ...
  virtual _Option::Result _assign(Token *token, std::ios::fmtflags flags,
    ...
    if (option_token != NULL) {
      ...
      do {
        ...
        if (result == _Option::OK) this->target.push_back(temp);
...
class _GlobalOption : public _Option {
  ...
  virtual Result operator()(ShortOptions &short_ops, LongOptions &long_ops,
                            Token *first, std::ios::fmtflags flags) const {
    // find first token GlobalArgument or UnknownYet (candidate) or
    // PossibleNegativeArgument (candidate too)
    Token *token(first);
    bool found(false);
    while (token != NULL && !found) {
      found = (token->type == Token::GlobalArgument ||
               token->type == Token::UnknownYet ||
               token->type == Token::PossibleNegativeArgument);
      if (!found) token = token->next;
    }
    if (found) {
 ...
 public:
  _GlobalOption(const _GlobalOption<T> &other) : target(other.target) {}

  _GlobalOption(T &target) : target(target) {}
};
...
class _GlobalOption<std::vector<T> > : public _Option {
  ...
  virtual Result operator()(ShortOptions &short_ops, LongOptions & /*long_ops*/,
    ...
    while (token != NULL && res == OK) {
      if (token->type == Token::GlobalArgument ||
          token->type == Token::UnknownYet ||
          token->type == Token::PossibleNegativeArgument) {
 ...
 public:
  ...
  _GlobalOption(std::vector<T> &target) : target(target) {}
};

template <class T>
inline _OptionT<T> Option(char short_opt, const std::string &long_opt,
                          T &target) {
...
inline _DefValOption<T, _OptionT<T> > Option(char short_opt,
                                             const std::string &long_opt,
                                             T &target, const T &def) {
  return _DefValOption<T, _OptionT<T> >(short_opt, long_opt, target, def);
}

template <class T>
inline _DefValOption<T, _OptionT<T> > Option(char short_opt, T &target,
                                             const T &def) {
...
//  no short opt.
template <class T>
inline _DefValOption<T, _OptionT<T> > Option(const std::string &long_opt,
                                             T &target, const T &def) {
  return _DefValOption<T, _OptionT<T> >(_Option::NO_SHORT_OPT, long_opt, target,
                                        def);
}

// Defaults for strings:
inline _DefValOption<std::string, _OptionT<std::string> > Option(
    char short_opt, const std::string &long_opt, std::string &target,
    const char *def) {
  return _DefValOption<std::string, _OptionT<std::string> >(short_opt, long_opt,
                                                            target, def);
}

inline _OptionT<std::string> Option(char short_opt, std::string &target,
                                    const char *def) {
  return _DefValOption<std::string, _OptionT<std::string> >(
      short_opt, std::string(), target, def);
}

//  no short opt.
inline _DefValOption<std::string, _OptionT<std::string> > Option(
    const std::string &long_opt, std::string &target, const char *def) {
  return _DefValOption<std::string, _OptionT<std::string> >(
      _Option::NO_SHORT_OPT, long_opt, target, def);
}
...
class OptionPresent : public _Option {
 ...
 public:
  ...
  OptionPresent(char short_opt) : short_opt(short_opt), present(NULL) {}

  // WITHOUT short_opt
  OptionPresent(const std::string &long_opt, bool &present)
      : short_opt(_Option::NO_SHORT_OPT),
        long_opt(long_opt),
        present(&present) {}
...
enum _EnvTag { Include_Environment };
...
class GetOpt_pp {
  ...
  static GETOPT_INLINE void _argc_argv_to_vector(
      int argc, const char *const *const argv, std::vector<std::string> &args);
 ...
 public:
  ...
  GETOPT_INLINE GetOpt_pp &operator>>(
  ...
  template <class T>
  inline T getopt(char short_opt, const std::string &long_opt,
                  const T &def_value) {
    T result;
  ...
  class _iterator {
   ...
   public:
    ...
    bool operator!=(
        const _iterator<Container, Adapter, OptionType> &other) const {
  ...
  typedef _iterator<LongOptions, LongAdapter, const std::string &>
      long_iterator;
};
