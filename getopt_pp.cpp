...
GETOPT_INLINE Token *GetOpt_pp::_add_token(const std::string &value,
                                           Token::Type type) {
...
GETOPT_INLINE void GetOpt_pp::_parse_sub_file(const std::string &file) {
  ...
  if (!ifile) throw OptionsFileNotFoundEx(file);

  std::vector<std::string> args;
  std::string arg;

  while (ifile >> arg) args.push_back(arg);
    ...
    if (currentArg[0] == '-' && currentArg.size() > 1) {
      ...
      if (currentArg[1] == '-') {
        if (currentArg.size() > 2) {
          // long option
          _longOps[currentArg.substr(2)].token =
              _add_token(currentArg.substr(2), Token::LongOption);
        } else {
          // it's the -- option alone
          _longOps[currentArg].token =
              _add_token(currentArg, Token::GlobalArgument);
        }

        any_option_processed = true;
      } else {
        // check if it is a negative number: rules
        //  * floating point negative numbers are straight classified as
        //  'arguments'
        //  * integer negative numbers of more than 1 digit length are also
        //  'arguments'
        //  * integer negatives of 1 digit length can be either arguments or
        //  short options.
        ...
        if (convert(currentArg, anInt, dummy.flags()) == _Option::OK) {
          if (currentArg.size() > 2)  // if it's larger than -d (d=digit), then
                                      // assume it's a negative number:
            _add_token(currentArg, any_option_processed
                                       ? Token::UnknownYet
                                       : Token::GlobalArgument);
          else  // size == 2: it's a 1 digit negative number
            _shortOps[currentArg[1]].token =
                _add_token(currentArg, Token::PossibleNegativeArgument);
        } else if (convert(currentArg, aFloat, dummy.flags()) == _Option::OK)
          _add_token(currentArg, any_option_processed ? Token::UnknownYet
                                                      : Token::GlobalArgument);
        else {
          // short option
          // iterate over all of them, keeping the last one in currentData
          // (so the intermediates will generate 'existent' arguments, as of
          // '-abc')
          for (size_t j = 1; j < currentArg.size(); j++)
            _shortOps[currentArg[j]].token =
                _add_token(std::string(currentArg, j, 1), Token::ShortOption);
        }
...
GETOPT_INLINE void GetOpt_pp::_argc_argv_to_vector(
...
GETOPT_INLINE GetOpt_pp &GetOpt_pp::operator>>(const _Option &opt) throw(
    GetOptEx) {
  if (_last != _Option::ParsingError) {
    ...
    switch (_last) {
      ...
      case _Option::OptionNotFound:
        if (_exc & std::ios_base::eofbit) throw OptionNotFoundEx();
        break;

      case _Option::BadType:
        if (_exc & std::ios_base::failbit) throw InvalidFormatEx();
        break;

      case _Option::NoArgs:
        if (_exc & std::ios_base::eofbit) throw ArgumentNotFoundEx();
        break;

      case _Option::TooManyArgs:
        if (_exc & std::ios_base::failbit) throw TooManyArgumentsEx();
        break;
...
GETOPT_INLINE bool GetOpt_pp::options_remain() const {
  ...
  if (!remain) {
    ...
    while (!remain && token != NULL) {
      remain = (token->type == Token::GlobalArgument ||
