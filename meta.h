...
class formula_generator {
 public:
  static inline void EXEC(result *p_result, int elcount, int minimum[],
...
#pragma omp parallel for
      for (k = minimum[N - 1]; k <= c; k++) {
        ...
        formula_generator<N - 1>::EXEC(p_result, elcount, minimum, maximum,
                                       masses, current_i, current_mass_i,
                                       loMass, hiMass, k);
        delete[] current_i;
      }
      ...
      for (int i = minimum[N - 1]; i <= c; i++) {
        ...
        formula_generator<N - 1>::EXEC(p_result, elcount, minimum, maximum,
                                       masses, current, current_mass_i, loMass,
                                       hiMass, k);
      }
    }
  }
