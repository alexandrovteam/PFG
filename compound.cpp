  ...
  if (countC) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (strcmp(rules[i], "NOPSC") == 0) {
        if (ratioNC > GLNOPSC[0] || ratioOC > GLNOPSC[1] ||
            ratioPC > GLNOPSC[2] || ratioSC > GLNOPSC[3]) {
          return false;
        }
      }

      if ((strcmp(rules[i], "NOPS") == 0) &&
          (countN > 1 && countO > 1 && countP > 1 && countS > 1)) {
        if ((countN >= 10) || (countO >= 20) || (countP >= 4) ||
            (countS >= 3)) {
          return false;
        }
      }
...
void help() {
  cout << "PFG is a program to calculate the possible elemental compositions "
          "for a given mass." << endl
...
int main(int argc, char *argv[]) {
  ...
  for (GetOpt_pp::short_iterator it = ops.begin(); it != ops.end(); ++it) {
    ...
    // std::cout << "\t" << short_opt << " has " << vec.size() << " integer
    // arguments." << std::endl;
    // vec.clear();
  }
  ...
  //		double  start, finish;
  //		start = clock();
  //		test(cmass[k], compositions, mincount, maxcount,
  // rules,GLHC,GLNOPSC,GLRDBE,3,"ppm",0,"H",1);
  //		finish = clock();
